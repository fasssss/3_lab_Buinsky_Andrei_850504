...
int main()
{
	...
	old_8 = getvect(0x80);      // IRQ0 прерывание таймера, возникает 18,2 раза в секунду.
	old_9 = getvect(0x90);      // IRQ1 прерывание от клавиатуры.
	old_10 = getvect(0xA0);     // IRQ2 используется для каскадирования аппаратных прерываний
	old_11 = getvect(0xB0);     // IRQ3 прерывание асинхронного порта COM2.
	old_12 = getvect(0xC0);     // IRQ4 прерывание асинхронного порта COM1.
	old_13 = getvect(0xD0);     // IRQ5 прерывание от контроллера жесткого диска для XT.
	old_14 = getvect(0xE0);     // IRQ6 прерывание генерируется контроллером флоппи диска после завершения операции
	old_15 = getvect(0xF0);     //IRQ7 прерывание принтера.
